...
#we need to initialize cross_domain model
class encResNet1d(nn.Module):
    """backbone + projection head"""
    def __init__(self,prior,in_channel=3, latent_dim=512,head='mlp'):
        super(encResNet1d, self).__init__()

        
        model_fun1, dim_in1 = model_dict['resnet18']

        self.imgencoder = model_fun1(in_channel=in_channel)
        # self.BETA=BETA
        self.prior=prior
        self.fc31 = nn.Linear(dim_in1,128)
        self.fc32 = nn.Linear(dim_in1,128)
        self.fc33 = nn.Linear(dim_in1,128)
        self.fc41 = nn.Linear(128,latent_dim)
        self.fc42 = nn.Linear(128,latent_dim)
        self.fc43 = nn.Linear(128,latent_dim)
        
        if head == 'linear':
            self.head_img_alpha = nn.Linear(dim_in1, latent_dim)
            self.head_img_beta = nn.Linear(dim_in1, latent_dim)
            

        elif head == 'mlp':
            self.head_img_alpha = nn.Sequential(
                nn.Linear(dim_in1, dim_in1),
                nn.ReLU(inplace=True),
                nn.Linear(dim_in1, latent_dim)
            )
            self.head_img_beta= nn.Sequential(
                nn.Linear(dim_in1, dim_in1),
                nn.ReLU(inplace=True),
                nn.Linear(dim_in1, latent_dim)
            )
    ...
    def forward(self, input_):
        ...
        image=self.imgencoder(input_)
        if self.prior=='beta':
            alpha=F.softplus(self.head_img_alpha(image))###
            beta=F.softplus(self.head_img_beta(image))
            return  alpha,beta
            
        elif self.prior=='normal':
            mu = F.relu(self.fc31(image))
            mu = self.fc41(mu)
            u = F.relu(self.fc32(image))
            u = torch.exp(self.fc42(u))
            return  mu,u
            
        elif self.prior=='base':
            mu = F.relu(self.fc31(image))
            mu = self.fc41(mu)
            u = F.relu(self.fc32(image))
            u = self.fc42(u)
            ...
            loc = F.relu(self.fc31(image))
            loc = self.fc41(loc)
            scale=F.softplus(self.head_img_alpha(image))
