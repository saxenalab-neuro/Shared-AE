...
import h5py
import random
from data import pose_neural_Dataset4
...
# param[0]=['image',256,1,0.07,30,2,5]#####weight:5, 3
# param[1]=['image',256,1,0.07,30,3,5]#####weight:5, 3
# param[2]=['image',256,1,0.07,30,4,5]#####weight:5, 3
# param[3]=['image',256,1,0.07,30,5,5]#####weight:5, 3
# param[4]=['image',256,1,0.07,30,6,5]#####weight:5, 3
...
hdf5_file=modelpath+'single_3dshapes.hdf5'
data1 = h5py.File(hdf5_file, 'r')
hdf5_file=modelpath+'single_3dshapes_label3.hdf5'
...
ll=1*len(list(data1.keys()))//40
ll2=1*len(list(data1.keys()))//10

for j in range(9):
    for i in range (ll2*j,ll2*j+ll):
        alllabels.append(str(i))
...
flag='models/m5_'+str(temperature)+'randomseed_'+str(param[jobid][2])+'weight_'+str(weight)+'dim_'+str(dims)+'_KS_'+str(dims)
if_label=False
######setup label####
# GN=47
# label=np.zeros((95))
...
dataset=pose_neural_Dataset4(alllabels,data1,data2,TwoDropTransform_twomodal(W,newW))
...
neural_dim=2#np.array(data2[alllabels[0]]).shape[-1]
